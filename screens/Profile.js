...
export default function Profile({person}) {
    ...
    const [income, setIncome] = useState(person.income)
    const [savings, setSavings] = useState(person.savings)
    const [targetToSave, setTargetToSave] = useState(person.targetToSave)
    const [totalExpenses, setTotalExpenses] = useState(person.totalExpenses)
    const [thisMonthStatus, setThisMonthStatus] = useState(income - totalExpenses - targetToSave)
    const [_id, set_id] = useState(person._id);
            ...
            <View style={{ flexDirection: 'row', height: 40, backgroundColor: "#F5F7F9" }}>
                ...
                <TouchableOpacity
                    style={{ 
                        flex: 1,
                        margin: 5,
                        backgroundColor: viewMode == "info" ? "#BEC1D2" : "#F5F7F9",
                        borderRadius: 5,
                    }}
                    onPress={() => {
